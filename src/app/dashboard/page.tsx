...
function DashboardContent() {
  ...
  const checkOAuthConfig = async () => {
    try {
      const response = await fetch('/api/auth?check=true');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Gmail Status & Login/Logout - Always visible */}
              <div className="flex items-center gap-2">
                ...
                {isLoggedIn && hasGmailData && (
                  <div className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400">
                    <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" />
                      <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
                    </svg>
                    <span>Dados Gmail</span>
                  </div>
                )}
                ...
                {isLoggedIn ? (
                  ...
                  /* Login Button or OAuth warning */
                  <>
                    {oauthConfigured === false && (
                      <div className="flex items-center gap-2 text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded-lg">
                        <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
                        <span className="hidden sm:inline">OAuth não configurado</span>
                      </div>
                    )}
                    {oauthConfigured !== false && (
                      <button
                      ...
                      >
                        ...
                        <span className="hidden sm:inline">Login Gmail</span>
                      </button>
                    )}
                  </>
                )}
